...
async def create_brain_upload_file(file: UploadFile = File(...)):
    #input: Brats_030_seg_brain.nii.gz
    global global_file_content
    global_file_content = await file.read()
    print("file read")
    # Save the file with a specific filename, such as "example.obj"
...
def get_obj_file():
    file_path = Path("D:/Pooja/Project School-2/Mern/mernweb/The Fast API App/lungs_obj_pred.obj")
    print(file_path)  # Add this line to print the file path
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="File not found")
    return FileResponse(path=file_path, media_type="application/octet-stream", filename="Lung_obj.obj")

@app.get("/get_brain_obj_file")
def get_brain_obj_file_route():
    return get_brain_obj_file()
def get_brain_obj_file():
    file_path = Path("D:/Pooja/Project School-2/Mern/mernweb/The Fast API App/brain_obj_pred.obj")
    print(file_path)  # Add this line to print the file path
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="File not found")
    return FileResponse(path=file_path, media_type="application/octet-stream", filename="brain_obj.obj")

@app.get("/get_heart_obj_file")
def get_heart_obj_file_route():
    return get_heart_obj_file()
def get_heart_obj_file():
    file_path = Path("D:/Pooja/Project School-2/Mern/mernweb/The Fast API App/heart_obj_pred.obj")
    print(file_path)  # Add this line to print the file path
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="File not found")
    return FileResponse(path=file_path, media_type="application/octet-stream", filename="heart_obj.obj")

@app.post("/open_unreal/")
async def open_unreal():
    # Open Unreal Engine project here
    open_unreal_command = "D:/Pooja/VR Radiology/MyProject2.uproject"
