class brain_model:
    def brain_pred(input_file):
        ...
        model.load_state_dict(torch.load("D:/Pooja/Project School-2/Mern/mernweb/The Fast API App/brain_best_metric_model.pth", map_location=torch.device('cpu')))
        model.eval()
        ...
        verts, faces, _, _ = measure.marching_cubes(selected_data, level=0, spacing=voxel_spacing)
            ...
            # Save mesh as OBJ
        with open('D:/Pooja/Project School-2/Mern/mernweb/The Fast API App/brain_obj_pred.obj', 'w') as obj_file:
            for v in verts:
